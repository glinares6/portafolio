    ...
    return (
        <>
            ...
            <div className="flex  flex justify-end  gap-x-1 items-center text-sm underline border-red-700 border-2">
                ...
                {search > 1 && (
                    <button onClick={() => handlePaginacion("page", `${currentPage - 1}`, currentPage - 1)}
                        className={'  w-9 h-8 '} >&lt;</button>
                )}
                {search > pageRange + 1 && (
                    <button onClick={() => handlePaginacion("page", "1", 1)} className="hover:bg-yellow-300 w-9 h-8" >1 </button>
                )}
                {search > pageRange + 2 && (
                    <span className={' flex justify-center  items-center hover:bg-yellow-300 w-9 h-8 '} >...</span>
                )}
                ...
                {search < totalPages - pageRange && (
                    <button onClick={() => handlePaginacion("page", `${totalPages}`, totalPages)}
