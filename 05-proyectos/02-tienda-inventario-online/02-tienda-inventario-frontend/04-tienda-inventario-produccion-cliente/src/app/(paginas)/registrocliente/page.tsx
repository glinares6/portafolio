...
export default function Page() {
  ...
  const { userPostAuth }: any = userApp();

  const { authState, setAuthState, cuentaState, pedidoList }: any =
    useContext(UseContext);

  const route = useRouter();

  useEffect(() => {
    if (pedidoList.pedidos) {
      if (pedidoList.pedidos.length > 0) {
        console.log("dato de la lista pedidos", pedidoList);
      }
    } else {
